...
class DataLoader:
    ...
    def __init__(self, data_dir=None):
        if data_dir is None:
            ...
            candidates = [
                here.parent / "data",           # BIST/data
                here / "data",                   # Simple Regime Filter/data
            ]
            for c in candidates:
...
def run_pipeline(data_dir=None, export=True, verbose=True):
    ...
    if verbose:
        print("=" * 70)
        print("SIMPLE REGIME FILTER")
        print("=" * 70)
        print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"Method:  Price vs 200 MA + Volatility Percentile")
        print(f"Regimes: Bull | Bear | Recovery | Stress")
        print("=" * 70)
    ...
    if verbose:
        ...
        for regime, row in dist.iterrows():
            ...
            print(f"  {regime:10s}: {row['Count']:5.0f} days ({row['Percent']:5.1f}%)  → {alloc:.0%} stocks")
        ...
        print(f"  Allocation: {current['allocation']:.0%} stocks")
        print(f"  Above MA:   {current['above_ma']}")
        print(f"  Vol %ile:   {current['vol_percentile']:.0f}th")
        print(f"  Vol:        {current['realized_vol']:.1%}")
    ...
    if verbose:
        ...
        print("-" * 63)
        print(f"{'Simple Regime Filter':<25} {results['annual_return']:>9.1%} "
              f"{results['sharpe_ratio']:>8.2f} {results['max_drawdown']:>9.1%} "
              f"{results['annual_volatility']:>7.1%}")
        print(f"{'Buy & Hold':<25} {bh_results['annual_return']:>9.1%} "
        ...
        print(f"Bull avg return:   {bull_ret:>7.1%}")
        print(f"Bear avg return:   {bear_ret:>7.1%}")
        print(f"Regime separation: {separation:>7.1%} {'✓' if separation > 0.30 else '⚠️'}")
