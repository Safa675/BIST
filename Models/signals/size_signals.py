"""
Size Signal Construction

Calculates composite size scores (INVERTED - smaller = better) based on:
1. Market Cap = log(Price × Shares)
2. Enterprise Value = log(Market Cap + Debt - Cash)
3. Revenue Size = log(Sales TTM)
4. Liquidity-Adjusted Size = log(Market Cap × ADV ratio)
"""
...
def build_size_signals(
    ...
    """
    Build composite size signal panel (INVERTED - smaller = higher score)
    
    Returns:
        DataFrame (dates x tickers) with inverted size scores
    """
    print("\n🔧 Building size signals...")
