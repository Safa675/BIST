...
class ReportGenerator:
    ...
    def save_results(
        ...
        output_path = self.models_dir / "results" / factor_name if output_dir is None else Path(output_dir)
    ...
    def save_correlation_matrix(
        ...
        if output_dir is None:
            output_dir = self.models_dir / "results"
        else:
    ...
    def save_capm_summary(factor_capm: dict, output_dir: Path | None = None, models_dir: Path | None = None):
        ...
        if output_dir is None:
            if models_dir is None:
                raise ValueError("models_dir is required when output_dir is None")
            output_dir = Path(models_dir) / "results"
        else:
            output_dir = Path(output_dir)
        output_dir.mkdir(parents=True, exist_ok=True)
