...
from Models.common.config_manager import REGIME_ALLOCATIONS
...
class Backtester:
    """Focused backtesting engine with reusable state + risk components."""
    ...
    def run(
        ...
        xautry_fwd_values = (
            prep.xautry_fwd_ret.reindex(loop_days).fillna(0.0).to_numpy(dtype=np.float64, copy=False)
        )
